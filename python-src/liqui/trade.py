...
from liqui import common
from liqui import keyhandler
...
class OrderInfo(object):
    ...
    def __init__(self, order_id, info):
        self.order_id = int(order_id)
        vnames = ("pair", "type", "start_amount", "amount", "rate", "timestamp_created",
                  "status")
        for n in vnames:
            setattr(self, n, info.get(n))
        self.timestamp_created = datetime.fromtimestamp(self.timestamp_created)
...
class TradeAPI(object):
    ...
    def orderInfo(self, order_id, connection=None):
        params = {"method": "OrderInfo",
                  "order_id": order_id}

        return OrderInfo(order_id, self._post(params, connection).get(str(order_id)))

    def depositAddress(self, symbol, connection=None):
        params = {"method": "CoinDepositAddress",
                  "coinName": symbol}

        return self._post(params, connection).get(u'address')

    def withdraw(self, symbol, amount, address, connection=None):
        params = {"method": "WithdrawCoin",
                  "coinName": symbol, "amount": float(amount), "address": address}
